...
import com.bailian.workbench.service.VideoT2vService;
...
public class VideoT2vController {

    private final VideoT2vService videoT2vService;

    public VideoT2vController(VideoT2vService videoT2vService) {
        this.videoT2vService = videoT2vService;
    }

    /**
     * Submit text-to-video task (Wan 2.7 T2V) and return taskId immediately.
     * Frontend should poll /api/v1/tasks/{taskId} for status.
     */
    @PostMapping("/t2v")
    public ResponseEntity<?> submit(@RequestBody VideoGenRequest request) {
        try {
            VideoGenResponse res = videoT2vService.submitTask(request);
