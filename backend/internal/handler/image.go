...
func HandleImageGen(ds *client.DashScope) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		params := map[string]any{"size": req.Size, "n": req.N}
		if req.Seed != nil { params["seed"] = *req.Seed }
		if req.Steps > 0 { params["steps"] = req.Steps }
		if req.PromptExtend != nil { params["prompt_extend"] = *req.PromptExtend }

		var body map[string]any
		if isQwen {
			content := []map[string]any{{"text": req.Prompt}}
			body = map[string]any{"model": req.Model, "input": map[string]any{"messages": []map[string]any{{"role": "user", "content": content}}}, "parameters": params}
		} else {
			input := map[string]any{"prompt": req.Prompt}
			if req.NegativePrompt != "" { input["negative_prompt"] = req.NegativePrompt }
			if req.RefImg != "" { input["ref_img"] = req.RefImg; if req.RefStrength > 0 { input["ref_strength"] = req.RefStrength } }
			body = map[string]any{"model": req.Model, "input": input, "parameters": params}
		}

		var resp *http.Response
		var apiErr error
		if m.Async { resp, apiErr = ds.DoAsync(m.Endpoint, body) } else { resp, apiErr = ds.Do(m.Endpoint, body) }
		if apiErr != nil { writeError(w, 500, "Image API error: "+apiErr.Error()); return }
		...
		if output, ok := result["output"].(map[string]any); ok {
			if tid, ok := output["task_id"].(string); ok { taskID = tid }
			if s, ok := output["task_status"].(string); ok { status = s }
		}
		if code, _ := result["code"].(string); code != "" { status = "failed" }

		// For sync Qwen images, download to local storage immediately
		savedBody := respBody
		if !m.Async && isQwen {
			savedBody = HandleSyncImageResult(ds, respBody)
		}

		repository.SaveGeneration("image", req.Model, req.Prompt, string(savedBody), taskID, status)
